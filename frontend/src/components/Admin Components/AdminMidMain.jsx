...
export function AdminMidMain(){
    ...
    const [category, setCategory] = useState(
        location.pathname === "/adminDashboard/hostel" ? "hostel" : "mess"
    );
    ...
    const handleResolved = (complaintId) => {
        setComplaints((prev) => prev.filter((comp) => comp._id !== complaintId));
    };
            ...
            <div className="w-full pl-[4%] pr-[2%] bg-stubgdark">
                ...
                <div className="h-[60px] sticky top-0 bg-stubgdark flex justify-end gap-x-4 items-center px-1 border-b-[1px] border-dashborder">
                    <FilterDropdown
                        ...
                        text2="Oldest"
                        text3="Most upvoted"
                        text4="Most downvoted"
                        hbgc="hover:bg-filterbtn/80"
                ...
                <div ref={containerRef} className="w-full flex flex-col gap-y-5 max-h-[calc(100vh-120px)] overflow-y-scroll scrollbar-thin scrollbar-webkit bg-stubgdark px-1 pt-1">
                    {complaints.map((item) => (
                        <AdminComplaintCard key={item._id} props={item} onResolve={handleResolved}/>
                    ))}
                    {isFetching && (
